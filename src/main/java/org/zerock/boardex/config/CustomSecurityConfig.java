...
@Log4j2
@Configuration  //환경설정 파일을 만듬
@RequiredArgsConstructor    //final이 붙거나 @NotNull이 붙은 필드이 생성자를 자동 생성해주는 lombok 어노테이션
...
public class CustomSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        ...
        //csrf : csrf 토큰 비활성화(아이디와 비밀번호 사용하여 로그인)
        http.csrf().disable();
        ...
        //tokenValiditySeconds() : 생성된 토큰의 만료 시간(여기선 30일)
        http.rememberMe().key("12345678")
                .tokenRepository(persistentTokenRepository())
                .userDetailsService(customUserDetailsService)
                .tokenValiditySeconds(60*60*24*30);
        //exceptionHandling() : Error 발생 시 후처리 설정
    ...
    //정식 사용자가 아닌 경우 게시글 수정하려고 접근했을 때 서버에서 접근 거부하면 예외처리함.
    @Bean
